...
function SymptomFormScreen() {
  ...
  useEffect(() => {
    const fetchURL = async () => {
      try {
        const response = await axios.get('https://cognitive-network-manager-rdwl5upzra-uw.a.run.app/');
    ...
    try {
      ...
      if (error.request && connectionAttempts <= 5) {
        const fetchData = async () => {
          try {
            const result = await axios.get('https://cognitive-network-manager-rdwl5upzra-uw.a.run.app/');
            url = result.data.url;
